...
import {ConditionalBlock, Operator} from "./blocs/conditional-bloc.class";
...
describe('Model Library', () => {
    ...
    it("State-based alarm", () => {
        ...
        const onConditionBlock: ConditionalBlock = new ConditionalBlock([button.isActive()], null, new CommandBlock([offState.callState()]), new CommandBlock([onState.callState()]))
        const offCondition: ConditionalBlock = new ConditionalBlock([button.isActive()], null, new CommandBlock([onState.callState()]), new CommandBlock([offState.callState()]))
        ...
        const application: Application = new Application([led, button], [onState, offState] , [offState.callState()]);
        console.log(application.export())
    })
    ...
    it("Multi-state alarm", () => {
        ...
        const offCommandBlock: CommandBlock = new CommandBlock([led.toDown(), buzzer.toDown()]);
        const noiseOnLedOffCommandBlock: CommandBlock = new CommandBlock([buzzer.toUp(), led.toDown()]);
        const noiseOffLedOnCommandBlock: CommandBlock = new CommandBlock([buzzer.toDown(), led.toUp()]);
        ...
        const from_noiseOnLedOff_to_noiseOffLedOn: ConditionalBlock = new ConditionalBlock(
            [button.isActive()],
            null,
        ...
        const application: Application = new Application([led, button, buzzer], [offState, noiseOnLedOffState, noiseOffLedOnState] , [offState.callState()]);
        console.log(application.export())
    })
    ...
    it("Temporal transitions", () => {
        ...
        const onCommandBlock: CommandBlock = new CommandBlock([led.toUp(), new DelayCommand(800), offState.callState()])
        ...
        const application: Application = new Application([button, led], [onState, offState], [offState.callState()])
        console.log(application.export())
    })
})
